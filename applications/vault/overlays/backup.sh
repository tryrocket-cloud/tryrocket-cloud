...
usage() {
  echo "Usage: $0 -u <healthcheck-uuid> -v <vault-version> [-h <hostname>] [-r <restic-version>]"
...
if [[ -z "$HC_UUID" || -z "$VAULT_VERSION" || -z "$RESTIC_VERSION" ]]; then
  usage
fi
...
restic backup --host "$HOSTNAME" --tag restic:"$RESTIC_VERSION" --tag vault:"$VAULT_VERSION" /vault/data
